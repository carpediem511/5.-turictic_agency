...
function renderTours(tours) {
                    ...
                    <div class="flex flex-col pb-10 pt-6 px-2 xl:px-10">
                        ...
                        <div class="flex flex-col mt-6 w-3/4 mx-auto">
                            ...
                            <button id="btnAddFavorite-${
                              tour.id
                            }" class="text-amber-500 font-medium drop-shadow-lg border justify-center border-sky-500 hover:text-white rounded-md px-3 py-2 hover:bg-fuchsia-400 transition-all duration-300">В избранное</button>
                            <button id="btnRemoveFromFavorites-${
                              tour.id
                            }"class="text-amber-500 font-medium drop-shadow-lg justify-center border border-sky-500 hover:text-white rounded-md px-3 py-2 hover:bg-red-600 transition-all duration-300">Удалить из избранного</button>
                        </div>
                    </div>
                </div>
        </div>
  ...
  tours.forEach((tour) => {
    ...
    ) //найти кнопку удалить из избранного
...
function openBookingWindow(id) {
  //ввожу функцию открыть модальное окно бронирования
...
document
  .getElementById("countriesFilter")
  .addEventListener("change", () => filterByCountry(tours))
...
document
  .getElementById("emptyStar2")
  .addEventListener("click", (event) => filterByRating(event, tours))
document
  .getElementById("emptyStar3")
  .addEventListener("click", (event) => filterByRating(event, tours))
document
  .getElementById("emptyStar4")
  .addEventListener("click", (event) => filterByRating(event, tours))
document
  .getElementById("emptyStar5")
  .addEventListener("click", (event) => filterByRating(event, tours))
